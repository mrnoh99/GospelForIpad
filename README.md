...
가라오케식 동기화 읽기 화면입니다. 본문에서 **절을 터치하면 오디오가 그 절로 이동**합니다(재생 중이면 즉시 탐색, 아니면 그 절부터 재생).

> ⭐️ 오디오 절 표식(`VerseTimestamps`)과 89개 오디오 파일은 **1급 데이터**입니다.
> 2026-07-08 실제 오디오(휴지 검출·강제 정렬)와 주교회의 성경 본문에 전량 대조·보정 완료 —
> 출처·방법론·수정 절차는 [`docs/AUDIO_TIMESTAMPS.md`](docs/AUDIO_TIMESTAMPS.md)를 따르고,
> 수정 후에는 `python3 scripts/validate_verse_timestamps.py`(가능하면 `--audio`)를 실행합니다.
